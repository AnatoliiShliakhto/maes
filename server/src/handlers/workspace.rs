...
async fn load_workspaces_meta() -> Result<Vec<WorkspaceMetadata>> {
    let Some(path) = Store::base_path() else {
        return Ok(vec![]);
    };

    let mut rd = fs::read_dir(&*path).await.map_err(map_log_err)?;
    let mut workspaces = Vec::new();

    while let Some(entry) = rd.next_entry().await.map_err(map_log_err)? {
        let file_type = entry.file_type().await.map_err(map_log_err)?;
        if !file_type.is_dir() {
            continue;
        }
        ...
        if path.exists() {
            let encrypted = fs::read(&path).await.map_err(map_log_err)?;
            let metadata = Store::decrypt_binary::<WorkspaceMetadata>(workspace, encrypted, false).await?;
