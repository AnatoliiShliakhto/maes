...
pub async fn get_task(
    ...
    match kind {
        EntityKind::Quiz => get_quiz_record_base(&session, task_id).await,
        EntityKind::Survey => get_survey_record_base(&session, task_id).await,
...
pub async fn delete_task(
    ...
    let kind = EntityKind::from_str(&kind).map_err(|_| (StatusCode::BAD_REQUEST, "bad-request"))?;
    if kind != EntityKind::Quiz && kind != EntityKind::Survey {
